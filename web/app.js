...
function catalogContext(catalog){for(const x of catalog||[]){for(const source of [x,x?.meta||{},x?.details||{}])for(const k of ['runtime_context_size','n_ctx','context_length','context_size','max_context_length'])if(source?.[k]!=null&&Number(source[k])>0)return Number(source[k]);}return null;}
...
function modelEls(role){return {model:$(role==='primary'?'primaryModel':'smallModel'),context:$(role==='primary'?'primaryContext':'smallContext'),hint:$(role==='primary'?'primaryModelHint':'smallModelHint'),status:$(role==='primary'?'primaryModelStatus':'smallModelStatus'),discover:$(role==='primary'?'discoverPrimaryBtn':'discoverSmallBtn'),test:$(role==='primary'?'testPrimaryBtn':'testSmallBtn')};}
function setModelStatus(role,message='',kind='neutral'){const el=modelEls(role).status;if(!el)return;el.textContent=message;el.className=`model-status ${kind}`;el.hidden=!message;}
function renderModelHint(role,catalog,contextOverride=null){const el=modelEls(role).hint,ctx=contextOverride||catalogContext(catalog),form=roleFormConfig(role),advertised=(catalog||[]).map(x=>x.id).filter(Boolean);const pieces=[];if(ctx)pieces.push(`Server runtime context ${Number(ctx).toLocaleString()}`);if(form.context_size)pieces.push(`form context ${Number(form.context_size).toLocaleString()}`);if(form.model&&advertised.length&&!advertised.includes(form.model))pieces.push('current model ID is not advertised by this endpoint');el.textContent=pieces.join(' · ')||'Endpoint catalog loaded.';}
function invalidateModelEndpoint(role){const e=modelEls(role);state.modelCatalogs[role]=[];fillDatalist(role,[]);e.model.value='';e.context.value='';e.hint.textContent='Endpoint changed — click Discover at Endpoint.';setModelStatus(role,'Discovery required for the new endpoint.','neutral');}
async function discoverModel(role){const e=modelEls(role),config=roleFormConfig(role),oldText=e.discover.textContent;e.discover.disabled=true;e.test.disabled=true;e.discover.textContent='Discovering…';setModelStatus(role,`Querying ${config.endpoint||'endpoint'}…`,'neutral');try{const data=await api('/models/discover',{method:'POST',body:JSON.stringify({role,config})});state.modelCatalogs[role]=data.catalog||[];fillDatalist(role,data.models||[]);if(data.status!=='AVAILABLE'||!(data.models||[]).length){e.model.value='';e.context.value='';renderModelHint(role,data.catalog||[],data.context_size);setModelStatus(role,data.error||`${role} endpoint unavailable`,'bad');throw new Error(data.error||`${role} endpoint unavailable`);}if(data.resolved_model)e.model.value=data.resolved_model;else if((data.models||[]).length===1)e.model.value=data.models[0];else if(!(data.models||[]).includes(e.model.value))e.model.value='';if(data.context_size&&Number(data.context_size)>0)e.context.value=Number(data.context_size);renderModelHint(role,data.catalog||[],data.context_size);const selection=e.model.value?`Resolved model: ${e.model.value}`:`${data.models.length} models found — select one.`;setModelStatus(role,`${selection}${data.context_size?` · Context: ${Number(data.context_size).toLocaleString()}`:''}`,'good');toast(`${role}: discovered ${data.models.length} model(s) at ${data.endpoint}.`);return data;}finally{e.discover.disabled=false;e.test.disabled=false;e.discover.textContent=oldText;}}
async function refreshModels(){const results=await Promise.allSettled([discoverModel('primary'),discoverModel('small')]);const failed=results.filter(x=>x.status==='rejected');if(failed.length)throw new Error(failed.map(x=>x.reason?.message||'Model discovery failed').join(' | '));return results;}
async function testModel(role){const e=modelEls(role),config=roleFormConfig(role),oldText=e.test.textContent;e.test.disabled=true;e.discover.disabled=true;e.test.textContent='Testing…';setModelStatus(role,`Testing ${config.model||'selected model'} at ${config.endpoint||'endpoint'}…`,'neutral');try{const r=await api('/models/test',{method:'POST',body:JSON.stringify({role,config})});if(r.catalog){state.modelCatalogs[role]=r.catalog;fillDatalist(role,r.models||[]);if(r.context_size&&Number(r.context_size)>0)e.context.value=Number(r.context_size);renderModelHint(role,r.catalog,r.context_size);}if(r.resolved_model&&!e.model.value)e.model.value=r.resolved_model;setModelStatus(role,r.message,r.ok?'good':'bad');toast(`${role}: ${r.message}`,!r.ok);return r;}finally{e.test.disabled=false;e.discover.disabled=false;e.test.textContent=oldText;}}
...
function wireActions(){
  ...
  $('primaryEndpoint').addEventListener('input',()=>invalidateModelEndpoint('primary'));$('smallEndpoint').addEventListener('input',()=>invalidateModelEndpoint('small'));
