# RCA Analyst POC RCA Core v0.8.6 Architecture

**Status:** candidate; not frozen  
**Carried by:** RCA Analyst application v1.8.7
...
No v0.8.6 fix introduces Python natural-language interpretation or weakens frozen evidence rules.
...
Critical semantic preparation and verification remain capacity-neutral roles: either may be routed to Small / Utility or Primary. The intended RunPod validation after v1.8.7 uses the stronger Primary 27B for both critical semantic roles while retaining utility tasks on the smaller model where appropriate.

## 3. Transport thinking control

Reasoning/thinking selection is a model-transport concern, not an RCA semantic rule.

When Thinking Off is selected for an OpenAI-compatible Qwen/llama.cpp request, v1.8.7 sends request-level chat-template control equivalent to:
...
## 5. Targeted structural completion

v0.8.6 replaces full-IR semantic regeneration with targeted field patches.
...
The completion call uses a compact bounded output budget because it is a repair, not a second full compilation.

## 6. Evidence semantic annotation
...
## 7. Evidence materiality

Materiality is determined from structured roles and dependencies, not prose similarity or requirement-ID association alone.

Narrative title/description/reported-result ambiguity is not automatically compliance-material.

Conversely, an unresolved fact remains material when its structured subject is actually used by Requirement IR and its semantics or required scope are unresolved. This ensures semantic uncertainty that can change compliance still blocks deterministic execution.

## 8. Independent semantic verification
...
v0.8.6 aligns verifier reconstruction with the current compiler contract for grounded NEQ predicates, plain literal comparisons and explicit persistence fields.

## 9. Semantic arbitration

At most one material semantic arbitration call is allowed before deterministic compliance.
...
A bare requirement violation or output symptom is insufficient to justify deep RCA. Primary RCA synthesis requires positive current-case mechanism evidence. Historical evidence remains supporting context only.

## 12. Frozen invariants

v0.8.6 preserves all previously accepted invariants, including:
...
RCA Core v0.8.6 is a **candidate**. Automated regressions are necessary but insufficient.

Live acceptance sequence:

1. TC17 must produce REQ-1701 APPLICABLE/VIOLATED, REQ-1702 NOT APPLICABLE, REQ-1703 NOT APPLICABLE, with no unsupported hypotheses.
2. Then TC12 must meet its documented ten-requirement target, including deterministic REQ-1204 1100 ms vs 800 ms (+300 ms).
