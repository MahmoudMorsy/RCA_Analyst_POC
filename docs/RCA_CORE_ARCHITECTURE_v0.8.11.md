# RCA Analyst POC RCA Core v0.8.11 Architecture

**Status:** candidate; not frozen  
**Carried by:** RCA Analyst application v1.8.13
...
No v0.8.11 fix introduces Python natural-language interpretation or weakens frozen evidence rules.
...
RCA Core v0.8.11 is a **candidate**. Automated regressions are necessary but insufficient.

Live acceptance sequence:

1. Deploy the exact v1.8.13 package and rerun the complete regression bundle with the same stable 27B critical-semantic routing used for the v1.8.12 validation run.
...
## v0.8.11 live-full-suite contract delta

The exact v1.8.12 / v0.8.10 RunPod session `RCA_20260903_110944_47ac48` completed all 17 cases and achieved 14/17 semantic acceptance. TEST-012 and TEST-017 remained clean anchors. TEST-007/015/016/018/021 passed after the v0.8.10 fixes. The remaining TEST-004/009/019 failures were traced to four deterministic integration contracts:

1. **Explicit repair targets are authoritative.** If a semantic issue already carries structured `target_fields`, Python uses exactly those fields. Human-readable verifier explanation text is not mined for more field names. Legacy description-based mapping remains fallback only when explicit targets are absent.
2. **Safe partial repair progress is retained.** Structural completion can apply valid targeted fields even when another target is omitted; the omitted field remains unresolved and is rediscovered in the next bounded pass. Arbitration likewise retains valid sibling target repairs. Changed untargeted fields are still rejected, and arbitration that produces no admissible target progress is conservatively rejected.
3. **RCA packet snapshot closure is correlation-based.** When a canonical current direct observation is selected for RCA, peers in the same explicit `observation_group` or at the exact same timestamp on the same explicit clock are included as structural context. This does not infer mechanism or causality and does not load the full trace.
4. **Historical evidence is non-normative by source boundary.** The deterministic compliance fact pool admits language-derived facts only from current direct observations, reported observations, or explicitly marked current-ticket scope metadata. Historical evidence, requirements, test instructions, hypotheses and ordinary ticket prose cannot establish current-case applicability or compliance.

These changes do not alter Requirement IR language authority, state/transition/interval semantics, correlation rules, persistence scope, deterministic timing, or Python final verdict authority.

### Live validation required

RCA Core v0.8.11 remains a candidate. Run the exact v1.8.13 package with the same verified 27B routing and require all frozen anchors plus the full 17-case acceptance suite before freezing.
